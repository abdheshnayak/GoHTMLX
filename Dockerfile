...
RUN go build -o /gohtmlx . && /gohtmlx --src=example/src --dist=example/dist

# Build example static assets (CSS + app.js) — Tailwind v4 needs tailwindcss in node_modules for @import
RUN cd example && npm install && mkdir -p dist/static && \
...
COPY --from=builder /build/example/dist/static /app/dist/static
EXPOSE 3000
CMD ["/app/example-app"]
